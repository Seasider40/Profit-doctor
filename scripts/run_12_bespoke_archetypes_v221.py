...
ARCH=ROOT/'qualification'/'bespoke_12_archetypes'
...
def make_archetype(case,d):
 import random
 random.seed(int(case.case_id[-3:]))
 customers=[('C1','Anchor Client','Enterprise'),('C2','Growth Client','Mid'),('C3','Stable Client','SME'),('C4','At Risk Client','Mid'),('C5','New Client','SME')]
 products=[('P1','Core A','Core','SUP1'),('P2','Core B','Core','SUP2'),('P3','Premium','Premium','SUP1'),('P4','Legacy','Legacy','SUP3')]
 write(d/'customers.csv',['customer_id','customer_name','segment','size'],[{'customer_id':x[0],'customer_name':x[1],'segment':x[2],'size':'Mid'} for x in customers])
 write(d/'products.csv',['product_id','product_name','family','supplier','base_price','base_cost'],[{'product_id':x[0],'product_name':x[1],'family':x[2],'supplier':x[3],'base_price':100+i*25,'base_cost':60+i*18} for i,x in enumerate(products)])
 rows=[]; tid=0
 for year in (2024,2025):
  for m in range(1,13):
   for ci,c in enumerate(customers):
    # planted truths differ by archetype
    if c[0]=='C4' and year==2025 and m>6: mult=.35 # churn/decline
    else: mult=1.0
    if c[0]=='C5' and year==2024: continue
    for pi,p in enumerate(products[:3] if case.business_model!='PROJECT_CONTRACT' else products[:2]):
     tid+=1; units=10+ci*3+pi*2
     price=100+pi*25
     cost=60+pi*18
     if case.business_model=='SUBSCRIPTION': units=1; price=900+ci*100; cost=250+pi*30
     elif case.business_model=='PROFESSIONAL_SERVICES': units=20+ci*5; price=150+pi*20; cost=75+pi*10
     elif case.business_model=='PROJECT_CONTRACT': units=1; price=(12000+ci*2500)*(1.8 if m in (3,7,11) else .45); cost=price*.58
     elif case.business_model=='HOSPITALITY_TRANSACTIONAL': units=80+ci*15; price=32+pi*6; cost=price*.34
     elif case.business_model=='HYBRID': price=130+pi*30; cost=price*.55
     # margin squeeze on P2 in H2 2025; partial price recovery
     if p[0]=='P2' and year==2025 and m>=7:
      cost*=1.18; price*=1.05
     # concentration: anchor larger
     units*= (3 if c[0]=='C1' else 1)
     units=max(1,int(units*mult))
     if c[0]=='C4' and year==2025 and m>6 and case.business_model in ('SUBSCRIPTION','PROJECT_CONTRACT'):
      price*=.35
     revenue=round(units*price,2); dc=round(units*cost,2); gp=round(revenue-dc,2)
     cts=round(revenue*(.035 if c[0]=='C1' else .055),2); contrib=round(gp-cts,2)
     delay=(22 if c[0]=='C4' and year==2025 else 3+ci)
     rows.append({'transaction_id':f'A{tid:06d}','month':f'{year}-{m:02d}-01','customer_id':c[0],'customer_name':c[1],'segment':c[2],'product_id':p[0],'product_name':p[1],'family':p[2],'supplier':p[3],'units':units,'unit_price':round(price,2),'unit_cost':round(cost,2),'revenue':revenue,'direct_cost':dc,'gross_profit':gp,'cts':cts,'contribution':contrib,'order_lines':1+pi,'picks':1+pi,'setups':1 if case.business_model in ('PROFESSIONAL_SERVICES','PROJECT_CONTRACT') else 0,'billing_delay_days':8 if case.business_model=='PROJECT_CONTRACT' else 2,'terms_days':30,'days_late':delay,'promo_flag':'False'})
 fields=['transaction_id','month','customer_id','customer_name','segment','product_id','product_name','family','supplier','units','unit_price','unit_cost','revenue','direct_cost','gross_profit','cts','contribution','order_lines','picks','setups','billing_delay_days','terms_days','days_late','promo_flag']
 write(d/'transactions.csv',fields,rows)
 truths=[
  {'truth_id':'T1','truth':'Anchor customer concentration','expected':'detect_or_expose'},
  {'truth_id':'T2','truth':'At Risk Client H2 2025 decline and slower payment','expected':'detect_or_expose'},
  {'truth_id':'T3','truth':'P2 cost inflation only partly recovered by price','expected':'detect_or_expose'},
  {'truth_id':'T4','truth':'New Client appears in 2025','expected':'detect_or_expose'},
 ]
 if case.business_model in ('PROFESSIONAL_SERVICES','SUBSCRIPTION','PROJECT_CONTRACT'): truths.append({'truth_id':'T5','truth':'Inventory economics not applicable','expected':'refuse_or_na'})
 write(d/'ground_truth.csv',['truth_id','truth','expected'],truths)
 return d
...
def accounting_files(d,case):
 ...
 if case.data_state in ('L2_ENHANCED','L3_ADVANCED'):
  ...
  with open(d/'transactions.csv',encoding='utf-8') as _fh:
   tx=list(_csv.DictReader(_fh))
 ...
 inv=0 if case.business_model in ('PROFESSIONAL_SERVICES','SUBSCRIPTION','PROJECT_CONTRACT') else round((sum(x[2] for x in revs) if case.data_state in ('L1_CORE','L1_MESSY') else direct)/365*58,2)
 bs_ar=ar*1.35 if case.case_id=='SME-011' else ar
 bs_cash=cash*1.7 if case.case_id=='SME-012' else cash
 bs_period='2025-12-31' if case.data_state in ('L2_ENHANCED','L3_ADVANCED') else '2026-12-30'
 bs=[{'period_end':bs_period,'line_code':'AR','line_name':'Trade Debtors','amount':bs_ar},{'period_end':bs_period,'line_code':'AP','line_name':'Trade Creditors','amount':ap},{'period_end':bs_period,'line_code':'CASH','line_name':'Cash','amount':bs_cash}]
 if inv: bs.append({'period_end':bs_period,'line_code':'INVENTORY','line_name':'Inventory','amount':inv})
 ...
 if case.case_id!='SME-011':
  ...
  write(d/'bank.csv',['transaction_id','transaction_date','amount','balance'],[{'transaction_id':'B1','transaction_date':('2025-12-30' if case.data_state in ('L2_ENHANCED','L3_ADVANCED') else '2026-12-29'),'amount':100,'balance':bankbal-100},{'transaction_id':'B2','transaction_date':('2025-12-31' if case.data_state in ('L2_ENHANCED','L3_ADVANCED') else '2026-12-30'),'amount':100,'balance':bankbal}])

def run_case(case,outdir):
 d=outdir/case.case_id; d.mkdir(parents=True,exist_ok=True); make_archetype(case,d); accounting_files(d,case)
 con=connect(d/'engine.db'); c='client_'+case.case_id; r='run_'+uuid.uuid4().hex
 con.execute('INSERT INTO client VALUES (?,?,?,?,?)',(c,case.case_id,'GBP',case.business_model,NOW));con.execute('INSERT INTO engine_run VALUES (?,?,?,?,?,?,?,?,?)',(r,c,'QUALIFICATION',NOW,None,'RUNNING',None,r,'2.21'));con.commit()
 ds=None; errors=[]
 # Enhanced/advanced receive transaction evidence. L1 intentionally does not.
 if case.data_state in ('L2_ENHANCED','L3_ADVANCED'):
  ing=ingest_northstar(con,c,r,d,d/'store'); ds=ing['dataset_version_id']; run_trust_layer(con,r,c,ds)
 ...
 run_primitive_engine(con,r,c,ds)
 if case.data_state=='L3_ADVANCED':
  # Genuine independent Level-3 planning/forecast/KPI evidence for qualification.
  con.execute('INSERT INTO plan_version VALUES (?,?,?,?,?,?,?,?,?)',(f'pv_{case.case_id}',c,'BUDGET','FY25 Budget','2024-12-15','2025-01-01','2025-12-31','ACTIVE','qualification budget'))
  for m in range(1,13):
   period=f'2025-{m:02d}'
   actual=100000+5000*m
   con.execute('INSERT INTO plan_line VALUES (?,?,?,?,?,?,?,?,?,?)',(f'pl_{case.case_id}_{m}',f'pv_{case.case_id}',c,period,'REVENUE',None,None,str(actual*1.06),'GBP','qualification budget'))
   con.execute('INSERT INTO actual_metric VALUES (?,?,?,?,?,?,?)',(f'am_{case.case_id}_{m}',c,period,'REVENUE',str(actual),'GBP','qualification actual'))
   con.execute('INSERT INTO forecast_vintage VALUES (?,?,?,?,?,?,?,?)',(f'fv_{case.case_id}_{m}',c,'2025-06-30',period,'REVENUE',str(actual*1.08),'GBP','qualification forecast'))
  con.execute('INSERT INTO kpi_observation VALUES (?,?,?,?,?,?,?,?,?,?)',(f'kpi_{case.case_id}',c,'2025-12','CONVERSION','Conversion Rate','31','PERCENT','COMMERCIAL','REVENUE','qualification KPI'))
  con.commit()
 diag=run_diagnostic_engine(con,r,c,ds); reason=run_reasoning_engine(con,r,c); econ=run_economic_engine(con,r,c)
...
def main():
 out=ROOT/'qualification'/'bespoke_12_execution'; shutil.rmtree(out,ignore_errors=True);out.mkdir(parents=True)
 results=[]
 for case in CASES:
